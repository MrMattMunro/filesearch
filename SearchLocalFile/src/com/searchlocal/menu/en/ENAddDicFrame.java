...
public class ENAddDicFrame extends CFrame implements ActionListener {

	private static final long serialVersionUID = 1L;

	static ENAddDicFrame c;
	...
	public ENAddDicFrame() {
		...
		panel.makeLabel(label_dic_note_r1, 29, 10, 400, 50);
		panel.makeLabel(label_dic_note_r2, 29, 30, 400, 50);
		panel.makeLabel(label_dic_note_r3, 29, 50, 400, 50);

		panel.makeLabel(label_dic_file, 32, 110, 110, 25);
		selectdir = panel.makeTextField(90, 110, 290, 20);
		selectdir.setEditable(false);

		JButton selectdirbotton = panel.makeButton(msg.getMsgbyId(Constant.button_select), 400,
				110, 60, 20, "");
		...
		coverdiccheckbox = panel.makeCheckbox(checkbox_iscoverdic, 29, 160, 240, 20, "");
		// 查看字典按钮
		viewdicbotton = panel.makeButton(msg.getMsgbyId(Constant.button_viedic), 270, 160, 140, 20,
				"");
		viewdicbotton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ENShowDicFrame viewDic = new ENShowDicFrame();
				try {
					viewDic.showframe();
		...
		// 确认
		JButton confirmbutton = panel.makeButton(msg.getMsgbyId(Constant.button_confirm), 170, 210,
				70, 20, "");
		...
		// 取消按钮
		JButton cancelbutton = panel.makeButton(msg.getMsgbyId(Constant.button_cancel), 250, 210,
				70, 20, "");
	...
	public static void main(String[] args) {
		try {
			...
			ENAddDicFrame frame = new ENAddDicFrame();
			frame.showframe();
		} catch (UnsupportedLookAndFeelException e) {
			// TODO 自动生成 catch 部分
			e.printStackTrace();
