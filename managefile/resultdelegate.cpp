...
void ResultDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if(index.data().canConvert<TableResult>())
        paintResult(painter, option, index);
    else
...
void ResultDelegate::paintResult(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    painter->drawText(textRect, Qt::AlignLeft|Qt::AlignVCenter, tp.filepath());

    painter->restore();
}
void ResultDelegate::paintLetter(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    const bool isSelected = option.state & QStyle::State_Selected;
    const bool isHovered = option.state & QStyle::State_MouseOver;
