...
//	char buf[100] = { 0 };
//	sprintf(buf, "取一个像素作为背景色 = RGB( %u, %u, %u)", GetRValue(color), GetGValue(color), GetBValue(color));
...
BOOL CBitmapHelper::PutPicture(IDispatch  *const pDisPtch, HBITMAP bBMP)
{
	...
	{
		OutputDebugStringA("[addin]  PutPicture--> Parameter Error");
		return FALSE;
	}
	...
	if ( SUCCEEDED( hr ) && (pd != NULL) )
	{
		...
		if ( FAILED(hr) ) 
		{
			OutputDebugStringA("[addin]  PutPicture--> Put_Invoke failed");
			return FALSE;
		}
		return TRUE;
	}
	...
	{
		OutputDebugStringA("[addin] PutPicture -->OleCreatePictureIndirect failed");
	}
	return FALSE;
}