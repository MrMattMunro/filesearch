...
public:
	...
	BOOL LoadWininet()
	{
		if (!mh_WininetDll)
		{
			...
			if (!mf_OpenW || 
			    ...
			    !mf_FtpGetFileSize) 
			{
				FreeLibrary(mh_WininetDll);
				mh_WininetDll = 0;
			}
		}
		return (mh_WininetDll != 0);
	}
	...
	DWORD SetUrl(WCHAR* u16_Url)
	{
		...
		if (!mf_CrackUrlW(u16_Url, 0, ICU_ESCAPE, &k_Comp))
			return GetInetError();
	...
	void AbortOperation()
	{
		...
		if (mh_WininetDll)
		{
			...
			mf_CloseHandle(mh_Connection);
			mf_CloseHandle(mh_Session);
			mf_CloseHandle(mh_InetFile);  // LAST !! WinInet.dll BUG !! (may block)
			mh_Connection = 0;
			mh_Session    = 0;
			mh_InetFile   = 0;
		}
	...
	DWORD ConnectServer(BOOL* pb_Offline)
	{
		*pb_Offline = FALSE;
		...
		mf_GetConectedState(&u32_StateFlags, 0);
		...
		mh_Session = mf_OpenW(L"ElmueSoft Cabinet Extraction", mu32_AccessType, u16_Proxy, 0, 0);
		if (!mh_Session)
			return GetInetError();

		mh_Connection = mf_ConnectW(mh_Session, mu16_Server, mu16_Port, mu16_Username, mu16_Password, mu32_Service, mu32_ConectFlags, 0);
		if (!mh_Connection)
			return GetInetError();
	...
	DWORD HttpOpenFile(WCHAR* u16_Method, void* p_POST, DWORD u32_POSTLength, WCHAR* u16_GET, DWORD* pu32_Status)
	{
		*pu32_Status = 0;
		...
		mh_InetFile = mf_HttpOpenRequestW(mh_Connection, u16_Method, s_Url, L"HTTP/1.1", 0, 0, mu32_ReqFlags, 0);
		if (!mh_InetFile)
			return GetInetError();
		...
		if (!mf_HttpSendRequestW(mh_InetFile, 0, 0, p_POST, u32_POSTLength))
			return GetInetError();
	...
	DWORD FtpOpenFile(DWORD u32_FirstByte)
	{
		...
		if (u32_FirstByte > 0)
		{
			...
			if (!mf_FtpCommandW(mh_Connection, FALSE, 0, u16_Command, 0, 0))
				return GetInetError();
		}
		...
		mh_InetFile = mf_FtpOpenFileW(mh_Connection, mu16_UrlPath, GENERIC_READ, FTP_TRANSFER_TYPE_BINARY|INTERNET_FLAG_DONT_CACHE, 0);
		if (!mh_InetFile)
			return GetInetError();

		DWORD u32_High;
		DWORD u32_Low = mf_FtpGetFileSize(mh_InetFile, &u32_High);
	...
	DWORD HttpQueryInfo(DWORD u32_InfoType, DWORD *pu32_Result)
	{
		*pu32_Result = 0;
		...
		if (!mf_HttpQueryInfoW(mh_InetFile, u32_InfoType, pu32_Result, &u32_Len, 0))
			return GetInetError();
	...
	DWORD ReadFile(void* p_Buffer, DWORD u32_Count, DWORD* pu32_Read)
	{
		*pu32_Read = 0;
		...
		do 
		{
			...
			if (!mf_ReadFile(mh_InetFile, pu8_Buf, u32_Chunk, &u32_Len))
				return GetInetError();
	...
	void CleanUp()
	{
		CloseInternet();
		DeleteTempFile();
		if (mh_WininetDll) FreeLibrary(mh_WininetDll);
	}
	...
	static BOOL ExplainInetErrorW(DWORD u32_Error, WCHAR* u16_Msg, DWORD u32_BufLen, DWORD u32_Status)
	{
		...
		if (u32_Error < INTERNET_ERROR_BASE || u32_Error > INTERNET_ERROR_LAST)
			return FALSE;

		HMODULE h_Wininet = GetModuleHandleW(L"Wininet.Dll");
		tGetLastResponseW f_GetLastResponseW = (tGetLastResponseW) GetProcAddress(h_Wininet, "InternetGetLastResponseInfoW");
		if (!f_GetLastResponseW)
			return FALSE;

		if (u32_Error == ERROR_INTERNET_EXTENDED_ERROR)
		{
			DWORD u32_ExtError;
			f_GetLastResponseW(&u32_ExtError, u16_Msg, &u32_BufLen);
		...
		if (FormatMessageW(FORMAT_MESSAGE_FROM_HMODULE, h_Wininet, u32_Error, 0, u16_Msg, u32_BufLen, 0))
			return TRUE;
	...
	BOOL HttpAddHeaders(WCHAR* u16_Headers, BOOL* pb_PartialContent)
	{
		*pb_PartialContent = FALSE;
		...
		while (TRUE)
		{
			...
			if (!mf_HttpAddHeadersW(mh_InetFile, u16_Start, u32_Len, HTTP_ADDREQ_FLAG_REPLACE | HTTP_ADDREQ_FLAG_ADD))
				return FALSE;
...
public:
	...
	DWORD DownloadEntireFileToDisk(DWORD *pu32_Status)
	{
		...
		*pu32_Status = 0;
		...
		do
		{
			if (u32_Err = ReadFile(u8_Buffer, sizeof(u8_Buffer), &u32_Read))
				return u32_Err;
