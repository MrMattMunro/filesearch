...
public class RegistTableUtil {
	...
	private static String EXCEL_PATH_CMD = REGQUERY_UTIL + "\"HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Office\\";
	...
	/** 根据JDK优先级来检索 */
	private static final String JAVAHOME_REGSTR_TOKEN = "JavaHome";
	
	/** 根据JDK优先级来检索*/
	private final static String[] JDK_VERSION = { "1.6.0_11", "1.6", "1.7", "1.5" };

	private static String JAVA_PATH_CMD = REGQUERY_UTIL + "\"HKEY_LOCAL_MACHINE\\SOFTWARE\\JavaSoft\\Java Runtime Environment\\";
	...
	public static String getAdobeInstallRoot() {
		...
		for (int i = 0; i < ADOBE_VERSION.length; i++) {
			try {
				Process process = Runtime.getRuntime().exec( ADOBE_PATH_CMD + ADOBE_VERSION[i] + "\\InstallPath" + "\"");
				StreamReader reader = new StreamReader(process.getInputStream());
	...
	public static String getExcelInstallRoot() {
		...
		for (int i = 0; i < OFFICE_VERSION.length; i++) {
			try {
		        Process process = Runtime.getRuntime().exec( EXCEL_PATH_CMD + OFFICE_VERSION[i] + "\\Excel\\InstallRoot" + "\"");
				StreamReader reader = new StreamReader(process.getInputStream());
