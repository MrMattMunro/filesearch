...
BOOL CStateTreeCtrl::ToggleCheckState(HTREEITEM hItem, UINT uFlags)
{
	...
	//如果选择的是父节点，则将所有子字节点全部选中
	if (ItemHasChildren(hItem))
	{
		//选择的是父节点，获取所有子节点，然后设置其状态
		SetItemCheck(hItem, nState);
		HTREEITEM hNextItem = NULL;
		hNextItem = GetNextItem(hItem);
		while (hNextItem != NULL && GetParentItem(hNextItem))
		{
			//hNextItem = GetNextItem(hItem);
			CString strText = GetItemText(hNextItem);
// 			if (hNextItem == NULL || ItemHasChildren(hNextItem))
// 			{
// 				break;
// 			}
			SetItemCheck(hNextItem, nState);
