...
//	std::string strQuerySQL = "select distinct path from t_recent_changeinfo where TO_DAYS(NOW())-TO_DAYS(systime) <= %d and operflg != 3 AND operflg != 4";
	char szSql[1024] = {0};
	sprintf(szSql, "select distinct path from t_recent_changeinfo where TO_DAYS(NOW())-TO_DAYS(systime) <= %d and operflg='2' Order By systime desc limit 0,300", nDays-1);
